...
let value = JSON.parse(localStorage.getItem('a2'));
document.querySelector('.out-3').innerHTML = `${key} ${value} <br>`
}
...
function t5() {
    let value = JSON.parse(localStorage.getItem('a4'));
    let allKey = '';

    function myKey() {
        for(key in value){
            allKey += `${key} ${value[key]} <br>`;
        };
    };
    myKey();
    document.querySelector('.out-5').innerHTML = allKey;
}
